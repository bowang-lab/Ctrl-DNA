...
def plot_four_metrics(
    ...
    for id, (ax, metric) in enumerate(zip(axes, metrics)):
        ...
        for model in r1.columns.levels[0]:
            mean = df[model]["mean"]
            std  = df[model]["std"]
        ...
        if id != 0 and metric != "de":
            for constraint, color in COLOR_MAP.items():
                ax.axhline(y=float(constraint), color=color, linestyle="--", linewidth=1)
            indicator = "↓"
        else:
            indicator = "↑"
    ...
    plt.tight_layout()
    plt.savefig(f"./visualize_results/{dataset}_ablation.png", dpi = 300, bbox_inches='tight')
    plt.show()
...
for dataid, dataset in enumerate(dataset_list):
    ...
    # r1.to_csv(f"./visualize_results/ablation_{dataset}_{task_list[0]}.csv")
    # r2.to_csv(f"./visualize_results/ablation_{dataset}_{task_list[1]}.csv")
    # r3.to_csv(f"./visualize_results/ablation_{dataset}_{task_list[2]}.csv")
    # de.to_csv(f"./visualize_results/ablation_{dataset}_de.csv")

    r1 = pd.read_csv(f"./visualize_results/ablation_{dataset}_{task_list[0]}.csv", index_col=0, header=[0, 1])
    r2 = pd.read_csv(f"./visualize_results/ablation_{dataset}_{task_list[1]}.csv", index_col=0, header=[0, 1])
    r3 = pd.read_csv(f"./visualize_results/ablation_{dataset}_{task_list[2]}.csv", index_col=0, header=[0, 1])
    de = pd.read_csv(f"./visualize_results/ablation_{dataset}_de.csv", index_col=0, header=[0, 1])
    ...
    plot_four_metrics(dfs, dataset=dataset)